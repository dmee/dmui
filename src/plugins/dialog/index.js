	...
	class Dialog {
		...
		get dialog_foot_temp() {
			...
			return [
				...
				'<a href="javascript:void(0);" class="dm-btn dm-btn-primary ' + (!options.showCancel ? 'dm-btn-block' : '') + ' btn-ok">',
				options.okVal || '',
				'</a>',
				options.showCancel ? '<a href="javascript:void(0);" class="dm-btn dm-btn-danger btn-cancel">' + (options.cancelVal || '') + '</a>' : '',
				'</div>',
			].join('');
		}
	}
